...
export async function applyToDraft(opts: {
  postId: string;
  path?: string;
  /** May be a string, or a rebuilt children/markDefs array. */
  newValue?: unknown;
  /** Remove the node entirely instead of setting a value (paragraph deletion). */
  unset?: boolean;
  /**
   * Several set-patches applied in ONE mutation — used when a formatted block
   * is rebuilt, because children and markDefs must change together: applying
   * one without the other leaves spans pointing at missing link definitions.
   */
  sets?: Record<string, unknown>;
}): Promise<{ draftId: string; transactionId: string }> {
  const token = readToken();

  const { postId, path, newValue, unset, sets } = opts;
  const draftId = `drafts.${postId}`;

  const setMap: Record<string, unknown> | null =
    sets ?? (path !== undefined && !unset ? { [path]: newValue } : null);
  const pathsToValidate = unset ? [path ?? ""] : Object.keys(setMap ?? {});
  if (pathsToValidate.length === 0) {
    throw new Error("applyToDraft called with nothing to do — no path, sets or unset.");
  }
  // Guard: paths are generated by our own planner, but validate anyway so a
  // corrupted row can never smuggle arbitrary content into a mutation path.
  for (const p of pathsToValidate) {
    if (!/^[A-Za-z0-9_.[\]"=\-]+$/.test(p)) {
      throw new Error(`Refusing to patch: unsafe path ${JSON.stringify(p)}`);
    }
  }
...
export async function repairDraft(slug: string, apply: boolean): Promise<RepairResult> {
  ...
  blocks.forEach((b, i) => {
    ...
    b.children.forEach((child, ci) => {
      ...
      if (working !== original) {
        // Address the span by _key, not index — an index goes stale the moment
        // anything edits the block concurrently and the patch lands on the
        // wrong span. Index only as a last resort for keyless spans.
        const spanRef = child._key ? `_key=="${child._key}"` : String(ci);
        const path = `body[_key=="${b._key}"].children[${spanRef}].text`;
        repaired.push({
          where: `Block ${i + 1}`,
          path,
