...
function mapNewsletter(
  ...
  return {
    ...
    deterministicTrackedSignups: intOrNull(row["Est. NAU"] || row["eNAU"]),
    ...
    metadata: buildMetadata({
      ...
      actualClicks: intOrNull(row["Actual Clicks"]),
      eNAU: intOrNull(row["Est. NAU"] || row["eNAU"]),
...
function mapLinkedIn(
  ...
  return {
    ...
    actualClicks: intOrNull(row["Actual clicks"]),
    deterministicTrackedSignups: null,
    notes: (row["UTM Link"] || row["Note"] || "").trim() || null,
    contentUrl: (row["Live content"] || row["Content URL"] || row["Content Link"] || "").trim() || null,
    channelUrl: (row["LinkedIn URL"] || "").trim() || null,
    metadata: buildMetadata({
      impressions: intOrNull(row["Impressions"]),
      followers: intOrNull(row["Followers"]),
      comments: intOrNull(row["Comments"]),
      estClicks: intOrNull(row["Estimated Clics"]),
      eNAU: intOrNull(row["Est. NAU"] || row["eNAU"]),
      ctr: floatOrNull(row["CTR"]),
...
function mapGeneric(
  ...
  return {
    ...
    deterministicTrackedSignups: intOrNull(
      row["Est. NAU"] || row["eNAU"] || row["deterministic_tracked_signups"],
    ),
    notes: null,
