...
- **Apply Addendum 3.** Your remedy is spliced into the live document by machine, so scope must be exact. A \`sentence\` rewrite replaces ONLY the quoted sentence and must read correctly between its neighbours; a \`paragraph\` rewrite replaces the WHOLE paragraph and must be written standalone. Mislabelling the scope produces a paragraph containing itself twice.
- Read the sentences either side of your anchor and do not repeat any phrase from them — calls to action and closing lines duplicate especially easily.
- Before proposing a sentence deletion, read what would remain. If removing the clause leaves broken punctuation or a fragment, choose paragraph scope or propose a rewrite instead, and say so in deletion_rationale.
